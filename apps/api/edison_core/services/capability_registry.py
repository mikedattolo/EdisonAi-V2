    ...
    def snapshot(self) -> CapabilityStatus:
        ...
        mcp_servers = [
            MCPServerRecord(
                ...
                tools=["knowledge.search", "knowledge.ingest_text", "knowledge.ingest_local", "knowledge.sources"],
            ...
            MCPServerRecord(
                ...
                tools=["workspace.search", "workspace.read", "workspace.preview_patch", "workspace.run_command"],
            ...
            MCPServerRecord(
                ...
                source="odysseus-inspired",
                detail="Connected to Edison media orchestration for ComfyUI, Invoke, Wan, and Modly.",
            ),
            MCPServerRecord(
                id="edison-camera",
                ...
                source="odysseus-inspired",
                detail="Camera is ready." if camera_ready else "No ready capture-capable camera was detected.",
            ),
            MCPServerRecord(
                id="edison-hardware",
                name="Edison Hardware MCP",
                status="ready" if hailo and hailo.status == "ready" else "staged",
                transport="stdio",
                description="Report GPU, fan, Hailo, camera, and storage state for agent planning.",
                tools=["hardware.status", "gpu.fans", "gpu.apply_fan_policy", "hailo.status"],
            ...
            MCPServerRecord(
                ...
                source="odysseus-inspired",
                detail="Local organizer and document stores are initialized.",
            ),
        ]
